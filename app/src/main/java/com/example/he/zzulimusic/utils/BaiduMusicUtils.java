package com.example.he.zzulimusic.utils;
...
public class BaiduMusicUtils {
    ...
    public static  byte[] readInputStream1(InputStream inputStream) throws IOException {
        byte[] buffer = new byte[1024];
        int len = 0;
    ...
    private static List<Mp3Info> parseJson(String ret) {
        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                info.setBigPicUri(object.getString("pic_big"));
                info.setUrl(getMusicUrl(info.getId()));
                info.setLrcLink(object.getString("lrclink"));
                list.add(info);
    ...
    public static void downloadLrc(String link, String title) {
        int code;
        ...
        try {
            ...
            if(code == 200){
                InputStream is = conn.getInputStream();
                File file = new File(saveDir+"/"+title+".lrc");
                //获取自己数组
    ...
    public static File downloadMp3(String mp3Url,String title) {
        int code;
        File saveDir  = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                "/zzulimusic/music");
        if(!saveDir.exists()){
        ...
        try {
            ...
            if (code == 200) {
                InputStream is = conn.getInputStream();
                byte[] getData = readInputStream1(is);
                File file = new File(saveDir+"/"+title+".mp3");
                FileOutputStream fos = new FileOutputStream(file);
                fos.write(getData);
                if(fos!=null){
                    fos.close();
    ...
    public static File downloadPic(String picLink,String title) {
        int code;
        File saveDir  = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                "/zzulimusic/music");
        if(!saveDir.exists()){
        ...
        try {
            ...
            if (code == 200) {
                InputStream is = conn.getInputStream();
                byte[] getData = readInputStream1(is);
                File file = new File(saveDir+"/"+title+".jpg");
                FileOutputStream fos = new FileOutputStream(file);
                fos.write(getData);
                if(fos!=null){
                    fos.close();
